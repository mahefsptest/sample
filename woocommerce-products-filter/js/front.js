...
//        if ((jQuery('#filter_reset_val').val()) === '1') {
//            var applyweblink = fullurl.split('?')[0] + '?swoof=1&reset';
//            history.pushState({}, '', applyweblink);
//            jQuery('#filter_reset_val').val('0');
//        }else{
//           var applyweblink = fullurl.split('?')[0];
////            console.log('linkapplyrest '+applyweblink);
//            history.pushState({}, '', applyweblink); 
//        }
//        if (fullurl.split('?')[1] === 'swoof=1&reset') {
//            var applyweblink = fullurl.split('?')[0] + '?swoof=1&reset'
//;            console.log('linkapplylog '+applyweblink);
//            history.pushState({}, '', applyweblink);
//        }
        woof_submit_link(woof_get_submit_link());
        return false;
    ...
    //Reset the search filter - Normal 
    jQuery('.woo_reset_search_form').click(function () {
...
function woof_submit_link(link) {
    ...
    if (woof_is_ajax === 1) {
        ...
        jQuery('#loadingtxt').show();
        jQuery('#addcartpopup #afterloading').hide();
        jQuery('#addcartpopup .modal-header .close').hide();
        jQuery('#add_to_cart_popup').click();
        ...
        jQuery.post(woof_ajaxurl, data, function (content) {
            ...
            jQuery('ul.woof_redraw_zone').replaceWith(jQuery(content.form).find('.woof_redraw_zone'));
            woof_remove_empty_elements();
            woof_init_search_form();
            ...
            if (jQuery("#filterMenu .mega-dropdown").hasClass('open')) {                    
                    ...
                    jQuery('.filter-by-toggle.mega-toggle').attr('aria-expanded',false);
            }
            
            //Hide the Preloader
            jQuery('#loadingtxt').hide();
            jQuery('#addcartpopup .modal-header .close').click();
            
            //Reset the search filter - Ajax method 
//            jQuery('.woo_reset_search_form').click(function () {
//                jQuery(':input').removeAttr('checked');
//                jQuery("#filter_reset_val").val('1');
//                if (Object.keys(woof_current_values).length > 0) {
//                    jQuery.each(woof_current_values, function (index, value) {
////                        console.log('index ', index);
////                        console.log('value ', value);
//                        if (index == 'product_cat') {
...
//                }
//                //var fullurl = jQuery(location).attr('href');
//                //var weburl = fullurl.split('?')[0]+'?swoof=1&reset';
//                //console.log(weburl);
//                //history.pushState('data', '', weburl);
//                return false;
//            });
            jQuery('.woo_reset_search_form').click(function () {
                jQuery(':input').removeAttr('checked');
                var fullurl = jQuery(location).attr('href');
                var weburl = fullurl.split('?')[0]+'?swoof=1&page=1';
                if (Object.keys(woof_current_values).length > 0) {
                    jQuery.each(woof_current_values, function (index, value) {
//                        console.log('index ', index);
//                        console.log('value ', value);
                ...
                history.pushState('data', '', weburl);
                jQuery('.woo_submit_search_form').trigger('click');
                return false;
            });
...
//    console.log('currentval '+ Object.keys(woof_current_values));
//    console.log(link);
//    var fullurl = jQuery(location).attr('href');
//    if (fullurl.split('?')[1] === 'swoof=1&reset') {
//        var link = fullurl.split('?')[0] + '?swoof=1&reset';
